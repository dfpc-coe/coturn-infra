...
process.env.BUILDX_BUILDER = process.env.BUILDX_BUILDER || 'coturn-infra-multiarch';
...
function tak() {
    return new Promise((resolve, reject) => {
        const image = `${process.env.AWS_ACCOUNT_ID}.dkr.ecr.${process.env.AWS_REGION}.amazonaws.com/tak-vpc-${process.env.Environment}-coturn:${process.env.GITSHA}`;
        const $ = CP.exec(`
            docker buildx build \
